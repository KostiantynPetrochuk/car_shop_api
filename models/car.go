...
type Car struct {
	...
	CylinderCount int     `binding:"required"`
...
func (c *Car) Save() error {
	query := `INSERT INTO cars(vin, brand_id, model_id, body_type, mileage, fuel_type, year, transmission, drive_type, condition, engine_size, door_count, cylinder_count, color, image_names) 
	...
	err = stmt.QueryRow(c.VIN, c.BrandId, c.ModelId, c.BodyType, c.Mileage, c.FuelType, c.Year, c.Transmission, c.DriveType, c.Condition, c.EngineSize, c.DoorCount, c.CylinderCount, c.Color, c.ImageNames).Scan(&c.ID)
	if err != nil {
		return err
	}
